...
class DBHelper(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    companion object{
        ...
        private val SQL_CREATE_USER = "CREATE TABLE " + DBInfo.UserTable.TABLE_NAME +
                "("+DBInfo.UserTable.COL_EMAIL+" VARCHAR(200) PRIMARY KEY, " +
                DBInfo.UserTable.COL_PASS + " TEXT, " + DBInfo.UserTable.COL_FULLNAME + " TEXT);"
    ...
    fun RegisterUser(emailin: String, passin: String, fullnamein: String) {
        ...
        var sql = "INSERT INTO " + namatable + " (" + emailt + ", " + passt + ", " + fullnamet + ") " +
                "VALUES('" + emailin + "', '" + passin + "', '" + fullnamein + "')"
        db.execSQL(sql)
    }
    @SuppressLint("Range")
    fun cekUser(emailin: String): String {
        ...
        var jumlah = ""
        try {
            cursor = db.rawQuery("SELECT COUNT("+ DBInfo.UserTable.COL_EMAIL +") as jumlah FROM "
                    + DBInfo.UserTable.TABLE_NAME + " WHERE " + DBInfo.UserTable.COL_EMAIL + "=='"
                    + emailin +"'" , null)
    ...
    fun cekLogin(emailin: String, passin: String): String {
        ...
        try {
            cursor = db.rawQuery("SELECT COUNT("+ DBInfo.UserTable.COL_EMAIL +") as jumlah FROM "
                    + DBInfo.UserTable.TABLE_NAME + " WHERE " + DBInfo.UserTable.COL_EMAIL + "=='" +
                    emailin +"' AND " + DBInfo.UserTable.COL_PASS + "=='" + passin + "'" , null)
        } catch (e: android.database.SQLException) {
            db.execSQL(SQL_CREATE_USER)
            return "Tabel Dibuat"
        }
